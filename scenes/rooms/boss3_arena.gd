...
func _ready() -> void:
	...
	# HACK FOR TESTING BLINK: Give the player Blink so they can test it on Boss 3
	if not PowerManager.has_power("blink"):
		var blink_power = load("res://scenes/player/powers/blink.gd").new()
		PowerManager.add_child(blink_power)
		PowerManager.slot2 = blink_power
		PowerManager.equip_power(1, &"blink")
