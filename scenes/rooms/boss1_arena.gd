extends Node2D
## Boss 1 Arena - Hothead

@onready var boss = $Hothead
...
func _on_power_slot_changed(slot_idx: int, power_id: StringName) -> void:
	...
	if power_id == &"":
		...
		# Use the color from the actual power if loaded, else fallback
...
func _on_boss_phase_changed(new_phase: int) -> void:
	# Add a little visual flair when a phase changes (screen shake)
	player._apply_screen_shake(4.0)
	
func _on_boss_died() -> void:
	boss_health_bar.visible = false
	# Ritual trigger will go here eventually
...
func _on_player_died() -> void:
	pass # Player handles their own reload
